...
test('Rankings offers direct library search and makes comparison optional', () => {
  ...
  assert.doesNotMatch(rankings, /className="rank-score"|score \$\{score\}/)
  ...
  assert.match(styles, /\.rank-row-copy strong\s*\{[^}]*-webkit-line-clamp: 2/s)
  assert.doesNotMatch(styles, /\.rank-score/)
