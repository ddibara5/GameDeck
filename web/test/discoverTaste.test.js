...
test('Discover puts persistent search and Browse before For You', async () => {
  ...
  assert.match(source, /name="game-search"/)
  assert.match(source, /autoComplete="off"/)
  assert.equal((source.match(/className={`discover-search-action/g) || []).length, 1)
  assert.match(source, /className="discover-filter-count"/)
...
test('Discover consolidates standing preferences into the filter sheet', async () => {
  const tabSource = await readFile(new URL('../src/components/DiscoverTab.jsx', import.meta.url), 'utf8')
  const browseSource = await readFile(new URL('../src/components/DiscoverBrowse.jsx', import.meta.url), 'utf8')

  assert.doesNotMatch(tabSource, /hideOwnedToken/)
  assert.match(tabSource, /preferenceFilterCount/)
  assert.match(browseSource, />Library</)
  assert.match(browseSource, /Include library games/)
  assert.match(browseSource, /Hide library games/)
  assert.match(browseSource, /onFilterCountChange\?\.\(temporaryFilterCount\)/)
})
