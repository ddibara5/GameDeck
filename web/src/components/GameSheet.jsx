...
export default function GameSheet({ variant, game, onClose, inLibrary = false, onAsk, onMoreLikeThis }) {
  ...
  const metaText = [year, genreText, platformText].filter(Boolean).join('  Â·  ')
      ...
      >
        ...
        {metaText || rating != null ? (
          <div className="gs-meta">
            ...
            {rating != null ? <span className="gs-star">{'â˜…'} {rating}</span> : null}
          </div>
        ) : null}
            ...
            {storyPct != null ? (
              <div className="detail-row">
                ...
                <span className="detail-value">
                  {storyPct}% {'Â·'} {minutesToHhm(game.playtime_minutes)} of ~{Math.round(len / 60)}h
                </span>
              </div>
            ) : null}
            <div className="detail-row">
